...
export default function DashboardPage() {
  ...
  const handleCopyAffiliateLink = async () => {
    const affiliateLink = `${window.location.origin}/sign-up?ref=${user.affiliateCode}`
    try {
            ...
            <div className="flex-x y-align-top">
              <div>
                <div className="flex-x col-gap-3">
                  <div className="display-text-sm">Affiliate earnings</div>
                </div>
                <div className="spacer-11"></div>
                <div className="referrals-balance">${(user?.affiliateEarnings || 0).toFixed(2)}</div>
              </div>
